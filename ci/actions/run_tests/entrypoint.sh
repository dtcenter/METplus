...
docker run -e GITHUB_WORKSPACE -v $RUNNER_WORKSPACE:$DOCKER_DATA_DIR -v $WS_PATH:$GITHUB_WORKSPACE ${VOLUMES_FROM} --workdir $GITHUB_WORKSPACE $DOCKERHUBTAG bash -c "${pip_command};${command}"

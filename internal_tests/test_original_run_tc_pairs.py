...
class TestOriginalRunTcPairs(unittest.TestCase):
    def setUp(self):
    ...
    def get_config():
        if 'JLOGFILE' in os.environ:
            produtil.setup.setup(send_dbn=False,
                                 jobname='test orig run_tc_pairs',
                                 jlogfile=os.environ['JLOGFILE'])
        else:
            produtil.setup.setup(send_dbn=False,
                                 jobname='test orig run_tc_pairs')
        produtil.log.postmsg('unit test for original run_tc_pairs is starting')

...
class TestProdutilForMETplus:
    ...
    def test_get_expected_num_items_for_dir(self):
        ...
        expected_items = ['PROJ_DIR', 'METPLUS_BASE', 'MET_BASE',
                          'MET_INSTALL_DIR', 'OUTPUT_BASE', 'TMP_DIR',
    ...
    def test_get_all_items_for_dir(self):
        ...
        # for logging information.
        cur_function = sys._getframe().f_code.co_name
        fail_msg = cur_function + \
                   ' |FAIL: Mismatch in retrieved vs expected items from ' + \
                   'the [dir] section of all config files.'
        pass_msg = cur_function + ' |PASS: The same expected and retrieved ' + \
                                  ' items were retrieved from the [dir]' + \
                                  ' section of all config files.'
        section_of_interest = 'dir'
        retrieved_items = self.config.items(section_of_interest)
    ...
    def test_get_option_value_for_dir(self):
        # Verify that the option,value pair for certain items are correctly
        # retrieved from the [dir] section of our
        ...
        expected_items = {'PROJ_DIR': '/tmp',
                          ...
                          'PARM_BASE': '/home/some_user/METplus/parm',
                          'LOG_DIR': '/log_dir',
                          'TEST_DIR': '/usr/local/met-6.1',
        ...
        for retrieved_item in retrieved_items:
            if len(retrieved_items) == len(expected_items):
                ...
                try:
                    assert expected_items[retrieved_option] == retrieved_item[
                        1], fail_msg
                except AssertionError:
                    self.logger.info(fail_msg)
                    return
    ...
    def test_config_raw(self):
        ...
        retrieved_raw = self.config.getraw(section_of_interest,
                                           'TEST_CONFIG_RAW')
        try:

...
* Put new dataset into a directory that matches the use case directories, i.e. model_applications/<category> or
...
* Create a tarfile on your development machine with the new dataset. Make sure the tarball contains directories
  model_applications/<category> or met_test::

    tar czf <tarfile_name>.tgz model_applications/<category>

* If you have access to mohawk, copy over the tarfile to mohawk in /d2/projects/METplus/METplus_Data_Staging::
...
For an example on how to upload data to the ftp site see “How to Send Us Data” on the
...
* As the met_test user, create a new directory in the METplus_Data web directory named after the branch
...
* Verify that all of the old and new data exists in the directory that was created (i.e. model_applications/<category>)
...
* Copy the volume_mount_directories file from the develop directory into the branch directory.

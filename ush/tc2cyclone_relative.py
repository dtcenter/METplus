...
from produtil.run import batchexe, runstr
import sys
import os
import re
...
def main():
    ...
    """Convert MET TC-Pairs output (A-deck and B-deck track files) into
       format for SBU cyc-new.dat and match.dat. Save each
       cyc-new.dat,match.dat pairing into a subdirectory based on
       their corresponding YYYYMMDD under the directory specified
       in the constants_pdef config/param file.

    return: None, creates four files: cyc-new.dat, match.dat,
            cyc-count.dat, and mcount.dat"""
    ...
    for tc in tc_pairs:
        ...
        with open(tc, "r") as cur_tc_file:
            ...
            for line in cur_tc_file:
                ...
                # Convert the adeck_lon, adeck_lat into grid lon,lat
                # points on the SBU map if they aren't 'NA'
                if 'NA' == adeck_lat or 'NA' == adeck_lon:
                    continue

                grid_lon, grid_lat = lonlat2grid(adeck_lon, adeck_lat)

                cyc_column_elements = [cyclone_number, ' ', fcst_lead_hh,
                                       ' ', grid_lon, ' ', grid_lat, ' ',
                                       adeck_mslp]
                model_column_elements = [fcst_lead_hh, ' ', cyclone_number,
                                         ' ', cyclone_number, ' ']
                cyc_columns = '   '.join(cyc_column_elements)
                model_columns = '   '.join(model_column_elements)

                # If any other columns of interest contains 'NA',
                # skip to the next line.
                if(
                  'NA' in cyc_columns or 'NA' in model_columns or
                   bdeck_mslp == 'NA'
                   ):
                    continue

                # Now that we don't have any 'NA' values,
                # we can calculate the center pressure bias and
                # append that to the model_columns string.
                center_pressure_bias = int(adeck_mslp) - int(bdeck_mslp)
                logger.debug("\nReading from file:{},cyclone num:{}," +
                             "init_time:{}, fcst_lead:{}, lon:{}, lat:{},"
                             " AMSLP:{}, BMSLP:{}, Bias {}"
                             .format(tc, cyclone_number, init_time,
                                     fcst_lead_hh, adeck_lon, adeck_lat,
                                     adeck_mslp, bdeck_mslp,
                                     center_pressure_bias))
                model_columns = model_columns + ' ' + str(center_pressure_bias)

                # Open each file, retrieve data, write data, then close files.
                # Use a dictionary to keep track of the filename
                # (full path) to its associated date.
                cyc_dict = update_dictionary(cyc_dict, cur_date, cyc_filename)
                model_dict = update_dictionary(model_dict, cur_date,
                                               model_filename)
                logger.info("Writing to corresponding cyc_new.dat and " +
                            "model.dat files for {}".format(cur_date))
                logger.debug("Day: {},Cyc column: {}"
                             .format(cur_date, cyc_columns))
                try:
                    ...
                    with open(model_filename, 'a') as model_file:
                        ...
                        model_file.write("\n")
                    logger.debug("Day: {},Model column: {}"
                                 .format(cur_date, model_columns))
                except IOError:
                    logger.error("ERROR: Couldn't open or write to " +
                                 "output file")

            logger.info("INFO Finished creating output for " +
                        cur_tc_file.name + ", on to generating counts...")
...
def update_dictionary(cur_dict, cur_key, cur_file):
    ...
    if cur_key not in cur_dict:
        logger.debug("Adding new key {} and value: {} to dictionary"
                     .format(cur_key, cur_file))
...
def lonlat2grid(in_lon, in_lat):
    ...
    # Since SBU algorithm doesn't use the convention of negative lon for
    # west of the prime meridian, make
    # necessary lon conversion (eg -179.5 is 179.5W, converted to
    # 180.5 in SBU's convention)
    ...
    if numerical_lon < 0:
        ...
        logger.debug('lon adjusted for different convention: {}'
                     .format(str(lon)))
    else:
        lon = numerical_lon
    ...
    # Get the fractional portion of the lon and lat
    # (ie value to right of decimal point).
    ...
    # Determine an adjustment factor to add so we round the original
    # lon and lat consistently
    ...
    # NOTE: because we assign longitudes west of the Prime Meridian with
    # negative value, the grid_lon formula is
    # 1 + (westmost_lon - adj_lon)/dlon rather than
...
def get_rounding_factor(fractional_value):
    """ Returns 0.5, 0, or 1 based on where the fractional value lies
...
def get_counts(model_cyc_filename):
    """ Determine the number of lines in the specified file and
    return the count"""
    ...
    if count_str:
        ...
        logger.error('WARNING: Cannot get count for file ' +
                     model_cyc_filename)

    logger.debug("DEBUG model/cyc filename " + model_cyc_filename +
                 " = " + count)
    return count


def post_process(fulldir):
    # Iterate over all the created directories and determine the number
    # of lines in each output file and create the mcount.dat (for the
    # number of lines in the model-test.dat file) and the cyc-count.dat file

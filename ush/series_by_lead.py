...
#            os.system(series_analysis_cmd)
    ...
    for cur_var in var_list: 
            ...
            for cur_stat in stat_list:
                ...
                logger.info("|INFO|: Plotting range for " + cur_var + " " + cur_stat + ":  " + str(vmin) + " to " + str(vmax))
...
def get_nseries(nc_var_file, p, logger):
    ...
    if match:
        ...
        logger.error("current netCDF file: " + nc_var_file)
        sys.exit()
    ...
    try:
        ...
        log.error("ERROR cannot open the min text file")
...
def get_min_max(nc_var_files, cur_stat, p, logger):
    ...
    for cur_nc in nc_var_files:
        ...
        try:
            ...
            log.error("ERROR cannot open the max text file")
...
def get_var_ncfiles(cur_var, nc_list, logger):
    ...
    num_tot = len(nc_list)
    if num_var == num_tot:
        logger.error("ERROR: the number of netCDF for variable" + cur_var + " is an inconsistent value, exiting...")
        sys.exit()
    return var_ncfiles

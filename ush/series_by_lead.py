...
# Call as follows:
# @code{.sh}
# SeriesByLead.py [-c /path/to/user.template.conf]
# @endcode
#
...
class SeriesByLead(object):
    """! @brief SeriesByLead performs series analysis of paired data based on
         lead time and generates plots for each requested variable and
         statistic, as specified in a configuration/parameter file.
    """

    def __init__(self, config):
        # Retrieve any necessary values from the parm file(s)
        self.fhr_beg = config.getint('config', 'FHR_BEG')
        self.fhr_end = config.getint('config', 'FHR_END')
        self.fhr_inc = config.getstr('config', 'FHR_INC')
        self.fhr_group_beg_str = util.getlist(config.getstr('config',
                                                            'FHR_GROUP_BEG'))
        self.fhr_group_end_str = util.getlist(config.getstr('config',
                                                            'FHR_GROUP_END'))
        ...
        self.fhr_group_labels = util.getlist(config.getstr('config',
                                                           'FHR_GROUP_LABELS'))
        ...
        self.rm_exe = config.getexe('RM_EXE')
        self.ncap2_exe = config.getexe('NCAP2_EXE')
        self.ncdump_exe = config.getexe('NCDUMP_EXE')
        ...
        self.series_lead_filtered_out_dir = \
            config.getdir('SERIES_LEAD_FILTERED_OUT_DIR')
        self.series_lead_out_dir = config.getdir('SERIES_LEAD_OUT_DIR')
        self.tmp_dir = config.getdir('TMP_DIR')
        self.background_map = config.getbool('config', 'BACKGROUND_MAP')
        self.regrid_with_met_tool = \
            config.getbool('config', 'REGRID_USING_MET_TOOL')
        ...
        self.fcst_ascii_regex = \
            config.getstr('regex_pattern', 'FCST_ASCII_REGEX_LEAD')
        self.anly_ascii_regex = \
            config.getstr('regex_pattern', 'ANLY_ASCII_REGEX_LEAD')
        self.logger = util.get_logger(config)
        self.config = config
        self.series_anly_configuration_file = \
            config.getstr('config', 'SERIES_ANALYSIS_BY_LEAD_CONFIG_PATH')
        self.var_list = util.getlist(config.getstr('config', 'VAR_LIST'))
    ...
    """
        ...
        # modifying to if self.fhr_group_beg never sets fhr_by_range=False
        ...
        if fhr_diff == 0 and self.fhr_inc == 0:
            ...
            self.logger.error('ERROR: fcst range indicated with increment '
                              'of 0 hrs, please check the configuration file.'
                              '  Exiting...')
            sys.exit(1)
        ...
        if fhr_by_range:
            ...
            # FHR_GROUP_END.
            if start_list_len != end_list_len != label_list_len:
                self.logger.error("The number of forecast hour begin and end "
                                  "times and number of corresponding labels"
                                  " must be the same.  Exiting...")
                sys.exit(1)
        ...
        self.generate_plots(fhr_by_range)

        # Create animated gif
        self.create_animated_gifs(fhr_by_range)
    ...
    def get_netcdf_min_max(self, fhr_by_range, nc_var_files, cur_stat):
    ...
    @staticmethod
    def get_var_ncfiles(fhr_by_range, cur_var, nc_list):
        """! Retrieve only the netCDF files corresponding to this statistic
            and variable pairing.

            Args:
                @param fhr_by_range: The boolean value indicating whether
    ...
    def retrieve_nc_files(self, fhr_by_range):
        """! Retrieve all the netCDF files created by MET series_analysis.

        Args:
            @param fhr_by_range:  Boolean value, True if series analysis was
    ...
    def generate_plots(self, fhr_by_range):
        """! Generate the plots and animation GIFs for the series analysis
             results.

             Args:
                 @param fhr_by_range   The boolean flag which indicates
        ...
        # filtering.
        nc_list = self.retrieve_nc_files(fhr_by_range)
        ...
        for cur_var in self.var_list:
            ...
            nc_var_list = self.get_var_ncfiles(fhr_by_range, name, nc_list)
            if not nc_var_list:
            ...
            for cur_stat in self.stat_list:
                ...
                vmin, vmax = self.get_netcdf_min_max(fhr_by_range, nc_var_list,
                                                     cur_stat)
                msg = ("|INFO|[ " + cur_filename + ":" + cur_function +
                ...
                for cur_nc in nc_var_list:
                    ...
                    nseries = self.get_nseries(fhr_by_range, cur_nc)
    ...
    def create_animated_gifs(self, fhr_by_range):
        """! Creates the animated GIF files from the .png files created in
             generate_plots().

             Args:
                  @param fhr_by_range:  The boolean flag indicating whether
...
if __name__ == "__main__":
    ...
    try:
        ...
        produtil.log.jlogger.critical(
            'SeriesByLead failed: %s' % (str(e),), exc_info=True)
        sys.exit(2)

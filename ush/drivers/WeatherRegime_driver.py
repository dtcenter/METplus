...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__),
    os.pardir,os.pardir)))
sys.path.insert(0, "/glade/u/home/kalb/UIUC/METplotpy_feature_74/metplotpy/contributed/blocking_s2s")
...
def main():
    ...
    all_steps = ["REGRID","TIMEAVE","ELBOW","PLOTELBOW","CALCEOF","PLOTEOF","KMEANS","PLOTBLOCKS"]
    ...
    #Check to make sure the setps have the required pre-steps
    ...
    if ("PLOTELBOW" in steps_list_obs):
        elbow_plot_title = config.getstr('Blocking','OBS_ELBOW_PLOT_TITLE','Elbow Method For Optimal k')
        elbow_plot_outname = config.getstr('Blocking','OBS_ELBOW_PLOT_OUTPUT_NAME','obs_elbow')
        pwr.plot_elbow(K_obs,d_obs,mi_obs,line_obs_curve_obs)

    if ("PLOTELBOW" in steps_list_obs):
        elbow_plot_title = config.getstr('Blocking','FCST_ELBOW_PLOT_TITLE','Elbow Method For Optimal k')
        elbow_plot_outname = config.getstr('Blocking','FCST_ELBOW_PLOT_OUTPUT_NAME','fcst_elbow')
        pwr.plot_elbow(K_fcst,d_fcst,mi_fcst,line_obs_curve_fcst)

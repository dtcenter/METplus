#!/usr/bin/env python
'''
Program Name: plot_grid2grid_anom_tsmean.py
Contact(s): Mallory Row
Abstract: Reads mean forecast hour files from plot_grid2grid_anom_ts.py to make dieoff plots
...
while s <= nstats: #loop over statistics
     ...
     while m <= nmodels: #loop over models
         ...
         if m == 1:
             ...
             model_now_CI_file = plotting_out_dir+"/data/"+cycle+"Z/"+model_now+"/"+stat_now+"_CI_"+region+"_fcst"+fcst_var_name+fcst_var_level+"_obs"+obs_var_name+obs_var_level+".txt"
             ...
             if count_masked != len(model_now_stat_now_mean):
                 ...
                 logger.debug("Plotting "+stat_now+" lead forecst hour means for "+model_now+" - "+model_list[0])

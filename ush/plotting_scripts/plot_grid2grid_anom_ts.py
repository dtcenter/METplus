        ...
        if nrow == 0: #file blank if stat analysis filters were not all met
            ...
            if data_line_type == 'SAL1L2':
                ...
                if create_data_arrays:
                    fabar_models_dates = np.full([nmodels,total_days], np.nan)
                    oabar_models_dates = np.full([nmodels,total_days], np.nan)
                    foabar_models_dates = np.full([nmodels,total_days], np.nan)
                    ffabar_models_dates = np.full([nmodels,total_days], np.nan)
                    ooabar_models_dates = np.full([nmodels,total_days], np.nan)
                ...
                for d in range(total_days):
                    ...
                    if len(dd) == 1:
                        fabar_models_dates[m-1, d] = model_now_data.loc[dd[0]]['A']
                        oabar_models_dates[m-1, d] = model_now_data.loc[dd[0]]['B']
                        foabar_models_dates[m-1, d] = model_now_data.loc[dd[0]]['C']
                        ffabar_models_dates[m-1, d] = model_now_data.loc[dd[0]]['D']
                        ooabar_models_dates[m-1, d] = model_now_data.loc[dd[0]]['E']
            elif data_line_type == 'VAL1L2':
                #if first model, initialize partial sum data arrays for all models and all dates
                if create_data_arrays:
                    ufabar_models_dates = np.full([nmodels,total_days], np.nan)
                    vfabar_models_dates = np.full([nmodels,total_days], np.nan)
                    uoabar_models_dates = np.full([nmodels,total_days], np.nan)
                    voabar_models_dates = np.full([nmodels,total_days], np.nan)
                    uvfoabar_models_dates = np.full([nmodels,total_days], np.nan)
                    uvffabar_models_dates = np.full([nmodels,total_days], np.nan)
                    uvooabar_models_dates = np.full([nmodels,total_days], np.nan)
                    create_data_arrays = False
                #read data for current model
                #check for any missing data in current model from requested date span,
                #arrange data in chronological order, and put in array
                for d in range(total_days):
                    dd = np.where(model_now_dates_formatted == dates[d])[0]
                    if len(dd) == 1:
                        ufabar_models_dates[m-1, d] = model_now_data.loc[dd[0]]['A']
                        vfabar_models_dates[m-1, d] = model_now_data.loc[dd[0]]['B']
                        uoabar_models_dates[m-1, d] = model_now_data.loc[dd[0]]['C']
                        voabar_models_dates[m-1, d] = model_now_data.loc[dd[0]]['D']
                        uvfoabar_models_dates[m-1, d] = model_now_data.loc[dd[0]]['E']
                        uvffabar_models_dates[m-1, d] = model_now_data.loc[dd[0]]['F']
                        uvooabar_models_dates[m-1, d] = model_now_data.loc[dd[0]]['G']
    else:
...
while s <= nstats: #loop over statistics
    ...
    logger.debug("Saving image as "+plotting_out_dir+"/imgs/"+cycle+"Z/"+stat_now+"_fhr"+lead+"_fcst"+fcst_var_name+fcst_var_level+"_obs"+obs_var_name+obs_var_level+"_"+grid+region+".png")
    plt.savefig(plotting_out_dir+"/imgs/"+cycle+"Z/"+stat_now+"_fhr"+lead+"_fcst"+fcst_var_name+fcst_var_level+"_obs"+obs_var_name+obs_var_level+"_"+grid+region+".png", bbox_inches='tight')
    s+=1

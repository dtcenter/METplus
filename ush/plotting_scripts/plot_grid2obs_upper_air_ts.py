#!/usr/bin/env python
'''
Program Name: plot_grid2obs_upper_air_ts.py
...
if date_filter_method == 'Valid':
   sd = datetime.datetime(syear, smon, sday, cycle_int)+datetime.timedelta(hours=lead_int%24)
   ed = datetime.datetime(eyear, emon, eday, cycle_int)+datetime.timedelta(hours=lead_int%24)+datetime.timedelta(days=1)
     ...
     while m <= nmodels: #loop over models
          ...
          if os.path.exists(model_now_stat_file):
              ...
              if nrow == 0: #file blank if stat analysis filters were not all met
                   ...
                   if stat_now == 'avg':
                       model_now_fbar_dates_vals = np.ones_like(dates)*np.nan
                       model_now_obar_dates_vals = np.ones_like(dates)*np.nan
                   else:
                   ...
                   if fcst_var_name == 'UGRD_VGRD' or obs_var_name == 'UGRD_VGRD':
                       ...
                       uvoobar = parsum[:,6]
                       if stat_now == 'avg':
                           model_now_fbar_vals = np.ma.masked_invalid(uvffbar)
                           model_now_obar_vals = np.ma.masked_invalid(uvoobar)
                       elif stat_now == 'bias':
                       ...
                       oobar = parsum[:,4]
                       if stat_now == 'avg':
                           model_now_fbar_vals = np.ma.masked_invalid(fbar)
                           model_now_obar_vals = np.ma.masked_invalid(obar)
                       elif stat_now == 'bias':
                   ...
                   if stat_now == 'avg':
                       model_now_fbar_dates_vals = np.zeros_like(dates)
                       model_now_obar_dates_vals = np.zeros_like(dates)
                       for d in range(len(dates)):
                           dd = np.where(model_now_dates == dates[d])[0]
                           if len(dd) != 0:
                               model_now_fbar_dates_vals[d] = model_now_fbar_vals[dd[0]]
                               model_now_obar_dates_vals[d] = model_now_obar_vals[dd[0]]
                           else:
                               model_now_fbar_dates_vals[d] = np.nan
                               model_now_obar_dates_vals[d] = np.nan
                   else:
              ...
              if stat_now == 'avg':
                  ...
                  model_now_stat_now_dates_vals = np.ones_like(dates)*np.nan
          if stat_now == 'avg':
              model_now_fbar_dates_vals = np.ma.masked_invalid(model_now_fbar_dates_vals)
              ...
              save_mean_file = plotting_out_dir+"/data/"+cycle+"Z/"+model_now+"/fbar_mean_"+region+"_fcst"+fcst_var_name+fcst_var_level+"_obs"+obs_var_name+obs_var_level+".txt"
              ...
              save_mean.write(lead+' '+str(model_now_fbar_dates_vals.mean())+ '\n')
              save_mean.close()
              model_now_obar_dates_vals = np.ma.masked_invalid(model_now_obar_dates_vals)
              #write forecast hour mean to file
              if not os.path.exists(os.path.join(plotting_out_dir, "data", cycle+"Z", model_now)):
                 os.makedirs(os.path.join(plotting_out_dir, "data", cycle+"Z", model_now))
              save_mean_file = plotting_out_dir+"/data/"+cycle+"Z/"+model_now+"/obar_mean_"+region+"_fcst"+fcst_var_name+fcst_var_level+"_obs"+obs_var_name+obs_var_level+".txt"
              if os.path.exists(save_mean_file):
                 append_write = 'a' # append if already exists
              else:
                 append_write = 'w' # make a new file if not
              logger.debug("Writing "+model_now+" f"+lead+" mean to "+save_mean_file)
              save_mean = open(save_mean_file,append_write)
              save_mean.write(lead+' '+str(model_now_obar_dates_vals.mean())+ '\n')
              save_mean.close()
              ...
              if m == 1:
                  ...
                  if nrow > 0:              
                      logger.debug("Plotting obar time series")
                      ax.plot_date(dates,model_now_obar_dates_vals, color='dimgrey', ls='-', linewidth=2.0, marker='o', markersize=7, label='obs '+str(round(model_now_obar_dates_vals.mean(),2))+' '+str(nrow-1))
                      logger.debug("Plotting fbar time series for "+model_now)
                      ax.plot_date(dates,model_now_fbar_dates_vals, color=colors[m-1], ls='-', linewidth=2.0, marker='o', markersize=7, label=model_now+' '+str(round(model_now_fbar_dates_vals.mean(),2))+' '+str(nrow-1))
                  ax.grid(True)

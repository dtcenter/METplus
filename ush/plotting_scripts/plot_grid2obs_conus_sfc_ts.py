#!/usr/bin/env python
'''
Program Name: plot_grid2obs_conus_sfc_ts.py
...
stat_files_input_dir = os.path.join(stat_files_input_dir_base, "conus_sfc")
...
while s <= nstats: #loop over statistics
    ...
    if data_line_type == 'SL1L2':
        if stat_now == 'avg':
             stat_now_vals = np.ma.masked_invalid(fbar_models_dates)
             stat_now_vals_obar = np.ma.masked_invalid(obar_models_dates)
        elif stat_now == 'bias':
        ...
        if stat_now == 'avg':
            stat_now_vals = np.ma.masked_invalid(uvffbar_models_dates)
            stat_now_vals_2 = np.ma.masked_invalid(uvoobar_models_dates)
        elif stat_now == 'bias':
    ...
    if event_equalization:
        ...
        if stat_now == 'avg':
            stat_now_vals_obar = np.ma.mask_cols(stat_now_vals_obar)
    #plot
    m=1
    while m <= nmodels: #loop over models
        model_now = model_names[m-1]
        ...
        with open(save_mean_filename, append_write) as save_mean_file:
            if stat_now == 'avg':
                model_now_stat_now_vals_obar = stat_now_vals_obar[m-1,:]
        ...
        if stat_now == 'avg':
            if m == 1:
                ...
                ax.plot_date(dates,stat_now_vals_obar[0,:], color='dimgrey', ls='-', linewidth=2.0, marker='o', markersize=7, label='obs '+str(round(stat_now_vals_obar[0,:].mean(),2))+' '+str(len(model_now_stat_now_vals) - np.ma.count_masked(stat_now_vals_obar[0,:])))

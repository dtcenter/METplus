#!/usr/bin/env python
'''
Program Name: plot_grid2obs_conus_sfc_tsmean.py
Contact(s): Mallory Row
Abstract: Reads mean forecast hour files from plot_grid2obs_conus_sfc_ts.py to make dieoff plots
...
while s <= nstats: #loop over statistics
     ...
     while m <= nmodels: #loop over models
         ...
         if stat_now == 'avg':
             ...
             model_now_fbar_mean_file = plotting_out_dir+"/data/"+cycle+"Z/"+model_now+"/fbar_mean_"+region+"_fcst"+fcst_var_name+fcst_var_level+"_obs"+obs_var_name+obs_var_level+".txt"
             ...
             #get forecast hour mean file
             model_now_obar_mean_file = plotting_out_dir+"/data/"+cycle+"Z/"+model_now+"/obar_mean_"+region+"_fcst"+fcst_var_name+fcst_var_level+"_obs"+obs_var_name+obs_var_level+".txt"

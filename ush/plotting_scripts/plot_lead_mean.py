...
Abstract: Reads mean and CI files from plot_time_series.py to make dieoff plots
...
for plot_info in plot_info_list:
    ...
    for stat in stats_list:
        ...
        for model_info in model_info_list:
            ...
            if ci_method != 'NONE':
                if stat == 'fbar_obar':
                    diff_from_mean_data = model_mean_data[1,:]
                    if os.path.exists(CI_file):
                        ...
                        logger.warning("Model "+str(model_num)+" "+model_name
                                       +" with plot name "+model_plot_name
                                       +" file: "+lead_mean_file+" does not"
                                       +" exist")
                else:
                    if model_num == 1:
                        diff_from_mean_data = model_mean_data[0,:]
                    else:
                        if os.path.exists(CI_file):
                            nrow = sum(1 for line in open(CI_file))
                            if nrow == 0:
            ...
            if model_num == 1:
                ...
                if stat == 'fbar_obar':
                    ax1.plot(fcst_lead_timedeltas, model_mean_data[1,:],
                             color='#888888',
                             ls='-', linewidth=2.0,

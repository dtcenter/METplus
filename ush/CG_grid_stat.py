...
class CG_grid_stat(CommandGen):
    ...
    def run_at_time(self, init_time, lead, accum, ob_type, fcst_var):
        ...
        if model_type == 'HREF_MEAN' or model_type == "NATIONAL_BLEND":
            ...
            run_pcp_ob = CG_pcp_combine(self.p, self.logger)
...
#                                   model_type, fcst_var, True)
            ...
            model_path = self.find_model(model_type, lead, init_time)
        ...
        fcst_str = model_type+"_"+fcst_var+"_"+accum+"_THRESH"
        fcst_threshs = util.getlistfloat(self.p.getstr('config', fcst_str))
        obs_str = ob_type+"_"+fcst_var+"_"+accum+"_THRESH"
        ...
        if self.p.getbool('config', model_type+'_IS_PROB'):
            ...
            fcst_field += "{ name=\""+fcst_var+"_"+accum + \
                          "\"; level=\"(*,*)\"; cat_thresh=["
            for fcst_thresh in fcst_threshs:

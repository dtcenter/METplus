...
from RMM_OMI_util import find_input_files, find_times, compute_plot_times, read_rmm_eofs


def run_rmm_steps(inlabel, inconfig, spd, EOF1, EOF2, oplot_dir):
...
def main():
    ...
    if run_obs_rmm:
        run_rmm_steps('OBS', config, spd, EOF1, EOF2, oplot_dir)

    if run_fcst_rmm:
        run_rmm_steps('FCST', config, spd, EOF1, EOF2, oplot_dir)

...
def run_omi_steps(inlabel, inconfig, spd, eof1_input_files, eof2_input_files, oplot_dir):
    ...
    PC1, PC2 = cmi.omi(olr[0:ntim,:,:], time, spd, eof1_input_files, eof2_input_files)
...
def main():
    ...
    eofdir = config.getraw('compute_omi','EOF_INPUT_DIR','')
    olr_eoffile1 = os.path.join(eofdir,config.getraw('compute_omi','EOF1_FILENAME_TEMPLATE','eof{valid?fmt=%j}.txt'))
    olr_eoffile2 = os.path.join(eofdir,config.getraw('compute_omi','EOF2_FILENAME_TEMPLATE','eof{valid?fmt=%j}.txt'))
    eofconfig = config_metplus.replace_config_from_section(config,'compute_omi')
    use_init_eof =  is_loop_by_init(eofconfig)
    if use_init_eof:
        eofconfig.set('config','INIT_BEG',eofconfig.getstr('config','EOF_INIT_BEG'))
        eofconfig.set('config','INIT_END',eofconfig.getstr('config','EOF_INIT_END'))
    else:
        eofconfig.set('config','VALID_BEG',eofconfig.getstr('config','EOF_VALID_BEG'))
        eofconfig.set('config','VALID_END',eofconfig.getstr('config','EOF_VALID_END'))
    eof_time = find_times(eofconfig, use_init_eof)
    eof1_input_files = find_input_files(eof_time, olr_eoffile1)
    eof2_input_files = find_input_files(eof_time, olr_eoffile2)
    ...
    # Observations
    if run_obs_omi:
        run_omi_steps('OBS', config, spd, eof1_input_files, eof2_input_files, oplot_dir)

    # Forecast
    if run_fcst_omi:
        run_omi_steps('FCST', config, spd, eof1_input_files, eof2_input_files, oplot_dir)

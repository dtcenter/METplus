...
import sys
sys.path.insert(0, '/usr/local/python3/lib/python3.7/site-packages')
sys.path.insert(0, '/usr/local/python3/lib/python3.7/site-python')
...
########################################################################
##
##  This script processes IMERG V06 NetCDF4 data files:
##  https://pmm.nasa.gov/data-access/downloads/gpm
##
##  Expected naming convention example:
##    3B-HHR.MS.MRG.3IMERG.20180807-S143000-E145959.0870.V06B.HDF5.nc4
##
##  Usage: sum_IMERG_V06_NC4.py input_dir var_name valid_time accum_hr
...
def read_IMERG_data(cur_file, var_name):
  ...
  # Read input file
  f = Dataset(cur_file, 'r')

  # Read the requested variable
  d = f[var_name]

  # Check for expected dimensions (time, lon, lat)
  #if d.coordinates != b'time lon lat':
  #  print("Unexpected dimensions: ", d.coordinates)
  #  sys.exit(1)
  ...
  lat = np.float64(f['lat'][:])
  lon = np.float64(f['lon'][:])
  a = {
   'name':      var_name,
   'long_name': var_name,
   'level':     'Surface',
   ...
   'grid': {
     'name':       'IMERG-Grid',
     'type' :      'LatLon',
     'lat_ll' :    round(min(lat), 4),
     'lon_ll' :    round(min(lon), 4),
  ...
  try:
    ...
    while cur_accum < accum_hr:
      ...
      cur_glob = input_dir + "/" + "*IMERG." + \
                 ...
                 (cur_valid - dt.timedelta(seconds=1)).strftime("-E%H%M%S") + "*.HDF5.nc4"
    ...
    print("Must specify exactly 4 arguments: input_dir, var_name, valid_time (YYYYMMDDHH), accum_hr")
    sys.exit(1)

...
class CyclonePlotterWrapper(CommandBuilder):
    ...
    def retrieve_data(self):
        """! Retrieve data from track files.
            Returns:
               None
               assigns value to the useful class variables: unique_storm_ids, track_df, pts_by_track_dict
        """
        ...
        if os.path.isdir(self.input_data):
            ...
            # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            ##!!!!USING PANDAS DATAFRAME!!!! GITHUB ISSUE METPLUS #1000
            # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            ...
            combined_df = combined.dropna(axis=0, how='any',
                                          subset=['ALAT', 'ALON', 'STORM_ID', 'LEAD', 'INIT', 'AMODEL', 'VALID'])
        ...
        # store the sanitized dataframe, and remove unecessary dataframes
        self.sanitized_df = sanitized_df
        del user_criteria_df
